...
import { CategorySelectField } from "@/features/products/components/category-select-field";
...
export const ProductForm = ({ productId, initialValues }: ProductFormProps) => {
  ...
  const onSubmit = async (values: ProductFormValues) => {
    // categoryId is non-empty here — the schema rejects "" before submit runs.
    const missing = missingRequiredAspects(
      values.categoryId as CategoryId,
      values.aspects,
    );
    if (missing.length > 0) {
      for (const aspect of missing) {
        form.setError(`aspects.${aspect}`, {
